...
class SecurityRulesAnalyzer {
  ...
  static Future<void> analyzeRules() async {
    ...
    try {
      ...
      final requirements = <String, bool>{
        'Helper functions for authentication': _checkHelperFunctions(rulesContent),
        'Role-based access control (admin, manager, worker)': _checkRoleBasedAccess(rulesContent),
        ...
        'Worker data access restrictions': _checkWorkerPermissions(rulesContent),
        'Time entry location validation': _checkLocationValidation(rulesContent),
        'Time entry type validation': _checkTimeEntryValidation(rulesContent),
        'Cross-company access prevention': _checkCrossCompanyPrevention(rulesContent),
      ...
      print('📈 Compliance Rate: ${(passed / (passed + failed) * 100).toStringAsFixed(1)}%');
  ...
  static bool _checkLocationValidation(String content) {
    ...
    return (content.contains('timeEntries') || content.contains('time_entries')) &&
