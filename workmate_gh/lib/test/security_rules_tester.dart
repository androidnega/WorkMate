...
class SecurityRulesTester {
  ...
  static const String testWorker2Email = 'test-worker2@workmate.com'; // Different company
  ...
  Future<void> _testManagerPermissions() async {
    ...
    await _testOperation(
      'Manager reads own company',
      () async {
        final doc = await _firestore.collection('companies').doc(testCompanyId1).get();
        return doc.exists;
      },
    );
    ...
    await _testOperation(
      'Manager cannot read other companies',
      () async {
        try {
    ...
    await _testOperation(
      'Manager creates worker for own company',
      () async {
  ...
  Future<void> _testWorkerPermissions() async {
    ...
    await _testOperation(
      'Worker cannot read other users',
      () async {
        try {
          final snapshot = await _firestore.collection('users').get();
          return false; // Should fail
    ...
    await _testOperation(
      'Worker cannot create user accounts',
      () async {
        try {
    ...
    await _testOperation(
      'Worker cannot create companies',
      () async {
        try {
          await _firestore.collection('companies').add({
            'name': 'Worker Company',
  ...
  Future<void> _testCompanyIsolation() async {
    ...
    await _testOperation(
      'Worker cannot access other company data',
      () async {
        try {
          final doc = await _firestore.collection('companies').doc(testCompanyId2).get();
          return false; // Should fail
        } catch (e) {
          return e.toString().contains('PERMISSION_DENIED');
        }
  ...
  Future<void> _testTimeEntryValidation() async {
    ...
    await _testOperation(
      'Valid clock-out without location',
      () async {
        await _firestore.collection('time_entries').add({
          'userId': _auth.currentUser!.uid,
          'companyId': testCompanyId1,
    ...
    await _testOperation(
      'Invalid location coordinates fail',
      () async {
        try {
          await _firestore.collection('time_entries').add({
            'userId': _auth.currentUser!.uid,
  ...
  Future<void> _testOperation(String testName, Future<bool> Function() operation) async {
    try {
  ...
  Future<void> _authenticateAs(String email) async {
    try {
      ...
      await _auth.createUserWithEmailAndPassword(email: email, password: testPassword);
    }
  }
  ...
  void _printTestSummary() {
    ...
    debugPrint('Success Rate: ${total > 0 ? ((passed / total) * 100).toStringAsFixed(1) : 0}%');
